...
continuum_normalize_spectra!(all_spectra)
...
mask_scale_factors = [  1.0, 2, 4.0, 6.0, 8, 10, 12 ]
 ...
 for (i,mask_scale_factor) in enumerate(mask_scale_factors)
   ...
   ccf_mid_velocity=ccf_mid_velocity, calc_ccf_var=false, recalc=true)
   #local (ccfs, v_grid) = ccf_total(order_list_timeseries, line_list_merged, pipeline_plan, mask_scale_factor=mask_scale_factor, mask_type=:tophat,
   #ccf_mid_velocity=ccf_mid_velocity, recalc=true)
   alg_fit_rv = EchelleCCFs.MeasureRvFromCCFGaussian(frac_of_width_to_fit=5)
   local rvs_ccf = calc_rvs_from_ccf_total(ccfs, pipeline_plan, v_grid=v_grid, times = order_list_timeseries.times, recalc=true, bin_nightly=true, alg_fit_rv=alg_fit_rv)
...
mask_scale_factors2 = [  1, 2, 4.0, 8.0, 10, 12] #, 16, 18, 20, 22, 24, 26, 28, 30, 32 ]
 ...
 for (i,mask_scale_factor) in enumerate(mask_scale_factors2)
   ...
   rvs_ccf_expr = calc_rvs_from_ccf_total(ccfs_expr, ccf_vars_expr, pipeline_plan, v_grid=v_grid_expr, times = order_list_timeseries.times, recalc=true, bin_nightly=true)
...
mask_scale_factors3 = [  4.0, 8.0, 10, 12, 16, 18, 20] # , 22, 24 , 26, 28, 30, 32]
 ...
 rms_within_night_rvs3 = zeros(length(mask_scale_factors3))
  ...
  for (i,mask_scale_factor) in enumerate(mask_scale_factors3)
    ...
    rvs_ccf_expr = calc_rvs_from_ccf_total(ccfs_expr, pipeline_plan, v_grid=v_grid_expr, times = order_list_timeseries.times, recalc=true, bin_nightly=true)
    #((ccfs_expr, ccf_vars_expr), v_grid_expr) = ccf_total(order_list_timeseries, line_list_df, pipeline_plan, mask_type=:halfcos, mask_scale_factor=mask_scale_factor,  ccf_mid_velocity=ccf_mid_velocity, calc_ccf_var=true, recalc=true)
    #rvs_ccf_expr = calc_rvs_from_ccf_total(ccfs_expr, ccf_vars_expr, pipeline_plan, v_grid=v_grid_expr, times = order_list_timeseries.times, recalc=true, bin_nightly=true)
 ...
 rms_within_night_rvs4 = zeros(length(mask_scale_factors4))
   println("Starting Gaussian...")
    for (i,mask_scale_factor) in enumerate(mask_scale_factors4)
      ...
      #rvs_ccf_expr = calc_rvs_from_ccf_total(ccfs_expr, pipeline_plan, v_grid=v_grid_expr, times = order_list_timeseries.times, recalc=true, bin_nightly=true)
      ((ccfs_expr, ccfs_var_expr), v_grid_expr) = ccf_total(order_list_timeseries, line_list_df, pipeline_plan, mask_type=:gaussian, mask_scale_factor=mask_scale_factor,  ccf_mid_velocity=ccf_mid_velocity, calc_ccf_var=true , recalc=true)
      rvs_ccf_expr = calc_rvs_from_ccf_total(ccfs_expr, ccfs_var_expr, pipeline_plan, v_grid=v_grid_expr, times = order_list_timeseries.times, alg_fit_rv=alg_fit_rv, recalc=true, bin_nightly=true)
...
mask_scale_factors5 = [  #= 6.0,8.0, =# 10.0, 12.0, 14.0, 16.0, 18.0, 20.0]
  ...
  rms_within_night_rvs5 = zeros(length(mask_scale_factors5))
   ...
   println("Starting Super Gaussian...")
    for (i,mask_scale_factor) in enumerate(mask_scale_factors5)
      ...
      #alg_fit_rv = EchelleCCFs.MeasureRvFromCCFQuadratic()  # Why isn't this doing better than Gaussian?
      alg_fit_rv = EchelleCCFs.MeasureRvFromCCFGaussian()
      #(ccfs_expr, v_grid_expr) = ccf_total(order_list_timeseries, line_list_df, pipeline_plan, mask_type=:supergaussian, mask_scale_factor=mask_scale_factor,  ccf_mid_velocity=ccf_mid_velocity, recalc=true)
      #rvs_ccf_expr = calc_rvs_from_ccf_total(ccfs_expr, pipeline_plan, v_grid=v_grid_expr, times = order_list_timeseries.times, recalc=true, bin_nightly=true)
      ((ccfs_expr, ccf_vars_expr), v_grid_expr) = ccf_total(order_list_timeseries, line_list_df, pipeline_plan, mask_type=:supergaussian, mask_scale_factor=mask_scale_factor,  ccf_mid_velocity=ccf_mid_velocity, calc_ccf_var=true, recalc=true)
      rvs_ccf_expr = calc_rvs_from_ccf_total(ccfs_expr, ccf_vars_expr, pipeline_plan, v_grid=v_grid_expr, times = order_list_timeseries.times, recalc=true, bin_nightly=true)
