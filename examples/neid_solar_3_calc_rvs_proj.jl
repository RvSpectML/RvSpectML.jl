...
chunk_grids = map(c->RvSpectML.make_grid_for_chunk(chunk_list_timeseries,c,oversample_factor=oversample_fac_chunks), 1:num_chunks(chunk_list_timeseries) )
 spectra_all = RvSpectML.pack_chunk_list_timeseries_to_matrix_linear(chunk_list_timeseries,chunk_grids)
...
if make_plots
   ...
   plt = plot()
   plot!(plt,spectra_matrix.λ[idx_plt],(spectra_matrix.flux[idx_plt,:].-mean_in_plt)./std_in_plt,linecolor=:black, label=:none)
   plot!(plt,spectra_matrix.λ[idx_plt],(f_mean[idx_plt].-mean_in_plt)./std_in_plt,linecolor=:red, label="Standardized mean spectrum")
   plot!(plt,spectra_matrix.λ[idx_plt], deriv[idx_plt]./std(deriv[idx_plt]), label="Standardized deriv", linecolor=:green)
   display(plt)
end
...
println("Computing RVs using dflux/dlnlambda from chunks.")
 ...
 if make_plots
   plt = scatter(times_binned, rvs_1, yerr=σ_rvs_1, label="RV chunks", color=:blue)
 end
...
order_grids = map(c->RvSpectML.make_grid_for_chunk(order_list_timeseries,c,oversample_factor=oversample_fac_orders), 1:num_chunks(order_list_timeseries) )
  spectral_orders_matrix = RvSpectML.pack_chunk_list_timeseries_to_matrix_linear(order_list_timeseries,order_grids)
  if 2 <= num_spectra_to_bin <=20
...
if make_plots
  ...
  plt_λ = RvSpectML.get_λs(order_grids, idx_plt)
     ...
     local plt2 = scatter(plt_λ,spectral_orders_matrix.flux[idx_plt,:].-f_mean_orders[idx_plt],markersize=1, label=:none)
     ylabel!(plt2,"Residuals")
     local pltall = plot(plt,plt2,layout=(2,1))
     display(pltall)
end
...
println("Computing RVs using dflux/dlnlambda from orders.")
  ...
  println("# rms(ave_order_rvs)/√N = ", std(ave_order_rvs)/sqrt(length(ave_order_rvs)), "  <RMS RVs>/√N = ", mean(rms_order_rvs)/sqrt(length(ave_order_rvs)), "  <σ_RVs> = ", mean(mean_order_σrvs) )
  if make_plots
    histogram(abs.(ave_order_rvs),bins=40,label="|Ave Order RVs|")
    histogram!(rms_order_rvs./sqrt(length(rms_order_rvs)),bins=40,label="RMS Order RVs/√N")
    histogram!(mean_order_σrvs,bins=40,label="σ Order RVs")
    xlabel!("(m/s)")
    ylabel!("Counts")
  end

chunk_rms_cut_off = 1.0
  idx_good_chunks = findall(x-> x<= chunk_rms_cut_off*sqrt(length(rms_order_rvs)), rms_order_rvs)
...
if make_plots
   plt = plot()
   scatter!(plt,plt_times, rvs_1, yerr=σ_rvs_1, label="Equal weighted chunks", markersize=3, color=:blue)
