...
order_grids = map(c->RvSpectML.make_grid_for_chunk(order_list_timeseries,c), 1:num_chunks(order_list_timeseries) )
#spectra_matrix = RvSpectML.pack_chunk_list_timeseries_to_matrix_linear(order_list_timeseries,order_grids)

chunk_grids = map(c->RvSpectML.make_grid_for_chunk(chunk_list_timeseries,c), 1:num_chunks(chunk_list_timeseries) )
spectra_matrix = RvSpectML.pack_chunk_list_timeseries_to_matrix_linear(chunk_list_timeseries,chunk_grids)

fm_mean = calc_mean_spectrum(spectra_matrix.flux,spectra_matrix.var)

if make_plots
  global idx_plt = 2617:2929
  mean_in_plt = mean(fm_mean[idx_plt])
  std_in_plt = stdm(fm_mean[idx_plt],mean_in_plt)
  global plt = scatter(spectra_matrix.λ[idx_plt],(spectra_matrix.flux[idx_plt,:].-mean_in_plt)./std_in_plt,markersize=1.0,legend=:none)
  plot!(plt,spectra_matrix.λ[idx_plt],(fm_mean[idx_plt].-mean_in_plt)./std_in_plt,legend=:none,linecolor=:black)
  #global plt = scatter(spectra_matrix.λ[idx_plt],spectra_matrix.flux[idx_plt],markersize=1.5, legend=:none)
end

deriv = calc_mean_deriv(spectra_matrix.flux,spectra_matrix.var,spectra_matrix.λ,spectra_matrix.chunk_map)

if make_plots
        #plot!(plt,spectra_matrix.λ[idx_plt] ,fm_mean[idx_plt].-1.0, label="mean")
        plot!(plt,spectra_matrix.λ[idx_plt], deriv[idx_plt]./std(deriv[idx_plt]), label="deriv")
end

(rvs, σ_rvs) = RvSpectML.calc_rvs_from_taylor_expansion(spectra_matrix,mean=fm_mean,deriv=deriv)

if make_plots
   scatter(chunk_list_timeseries.times, rvs, yerr=σ_rvs)
end

rms_rvs = std(rvs)
 mean_σ_rvs = mean(σ_rvs)
 println("# rms(RVs) = ", rms_rvs, "  σ_RVs = ", mean_σ_rvs, "   N_obs = ", length(rvs) )
