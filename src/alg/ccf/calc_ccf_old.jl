"""
    Older version of code to compute CCFs, specialized for tophat masks of constant width.
    May be useful for comparisons/debugging
...
function ccf_1D_old(λ::A2, flux::A3, #line_list::ALL, #mask_shape1::A3
    ...
    @assert ndims(flux) == 1
    @assert length(λ) == length(flux)
    # Old version made additional assumptions about lines not overlapping
