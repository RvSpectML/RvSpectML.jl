...
=#
    ...
    flux_out = TemporalGPInterpolation.predict_mean(chunk.λ./boost_factor, y_to_use, grid, sigmasq_obs = var_to_use, use_logx=use_logx, use_logy=false, smooth_factor=smooth_factor )
    ...
    var_out .= TemporalGPInterpolation.predict_mean(chunk.λ./boost_factor, var_to_use, grid, sigmasq_obs = var_to_use, use_logx=use_logx, use_logy=false, smooth_factor=smooth_factor )
#=
    if !use_logy
...
function interp_chunk_to_grid_gp_temporal!( flux_out::AA1, var_out::AA2, chunk::AC, grid::AR ; use_logx::Bool = true,  use_logy::Bool = false, smooth_factor::Real=1 ) where { T1<:Real, AA1<:AbstractArray{T1,1}, T2<:Real, AA2<:AbstractArray{T2,1}, AC<:AbstractChunkOfSpectrum, AR<:Union{AbstractRange,AbstractArray{T2,1}} }
    ...
    flux_out .= TemporalGPInterpolation.predict_mean(chunk.λ, y_to_use, grid, sigmasq_obs = var_to_use, use_logx=use_logx, use_logy=false, smooth_factor=smooth_factor )
    # TODO: Update var_out to actually use the right GP
    var_out .= TemporalGPInterpolation.predict_mean(chunk.λ, var_to_use, grid, sigmasq_obs = var_to_use, use_logx=use_logx, use_logy=false, smooth_factor=smooth_factor )
    if !use_logy
        flux_out .= mean_y.*(flux_out.+1.0)
        var_out .*= mean_y^2
    else
